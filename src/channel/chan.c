...
void chan_dispose_global()
{
    static int global_dispose = 0;
    if (global_dispose) {
        return;
    }
    pthread_mutex_destroy(&s_mu);
...
static int unbuffered_chan_init(chan_t* chan)
{
    ...
    if (pthread_mutex_init(&chan->select_mutex, NULL) != 0)
    {
        pthread_mutex_destroy(&chan->m_mu);
        pthread_mutex_destroy(&chan->w_mu);
        pthread_mutex_destroy(&chan->r_mu);
...
void chan_dispose(chan_t* chan)
{
    ...
    pthread_cond_destroy(&chan->r_cond);
    pthread_cond_destroy(&chan->w_cond);
    pthread_mutex_destroy(&chan->select_mutex);
    free(chan);
}
...
static int buffered_chan_send(chan_t* chan, void* data)
{
    ...
    pthread_mutex_lock(&chan->select_mutex);
    if (chan->block_in_select)
    {
        chan->block_in_select = 0;
...
static int chan_candidates(select_op_t* candidates, chan_t* recv_chans[], int recv_count,
    ...
    chan_t* send_chans[], int send_count, void* send_msgs[])
{
    ...
    int count = 0;
    int i = 0;
    while ((count = chan_candidates(candidates, recv_chans, recv_count, send_chans, send_count, send_msgs)) == 0) {
        for (i = 0; i < recv_count; i++)
        {
            chan_t* chan = recv_chans[i];
            pthread_mutex_lock(&chan->select_mutex);
            chan->block_in_select = 1;
            pthread_mutex_unlock(&chan->select_mutex);
        }
        for (i = 0; i < send_count; i++)
        {
